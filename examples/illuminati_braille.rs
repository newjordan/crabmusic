//! Illuminati Braille Animation
//!
//! Slowly pulsing Illuminati symbol built in the Braille domain. The triangle
//! hums with a golden aura while the central eye blinks and tracks the viewer.
//! Everything renders via the existing braille grid utilities.
...
const FRAME_DURATION: Duration = Duration::from_millis(84);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Summoning illuminati braille animation… Press 'q' or ESC to exit.");
...
impl IlluminatiAnimation {
    ...
    fn draw_energy_line(
        ...
        let pulse_speed = 0.9 + strength * 1.8;
    ...
    fn draw_eye(
        ...
        let pupil_radius_x = eye_radius_x * (0.32 + (1.0 - open) * 0.08);
        let pupil_radius_y = eye_radius_y * (0.36 + (1.0 - open) * 0.12);
    ...
    fn draw_dripping_edge(
        ...
        for i in 0..drip_count {
            ...
            let growth = (phase.sin() * 0.5 + 0.5).powf(1.4);
    ...
    fn eye_open_amount(&self) -> f32 {
        ...
        let micro = 0.05 * (self.time * 0.9).sin() + 0.03 * (self.time * 0.37).sin();
        (main + micro).clamp(0.08, 1.0)
    }
    ...
    fn compose_color(
        ...
        if mix.glow > 0.0 {
            color = add_color(color, Color::new(170, 200, 120), (mix.glow * 0.55).min(1.0));
        }
        if mix.edge > 0.0 {
        ...
        if mix.highlight > 0.0 {
            color = blend(color, Color::new(255, 255, 255), mix.highlight.min(1.0));
        }
        if mix.drip > 0.0 {
    ...
    fn base_color(&self, x: usize, y: usize, width: usize, height: usize) -> Color {
        ...
        let base_r = 12.0 + swirl * 18.0 + pulse * 10.0;
        let base_g = 20.0 + swirl * 24.0 + (1.0 - vertical) * 22.0;
        let base_b = 28.0 + swirl * 35.0 + vertical * 18.0 + (1.0 - dist).max(0.0) * 24.0;
