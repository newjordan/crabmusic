// Mandala Generator Demo
// Visual test for the Mandala sacred geometry visualizer
...
use crabmusic::visualization::{
    ...
    GridBuffer, MandalaConfig, MandalaVisualizer, Visualizer,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🕉️  Mandala Generator Demo");
    println!("Press 'q' to quit, 'c' to cycle colors, 's' to change symmetry, '+/-' for layers");
    ...
    let mut viz = MandalaVisualizer::new(config.clone());
    ...
    // Symmetry options
    let symmetries = vec![4, 6, 8, 12];
    let mut symmetry_idx = 2; // Start with 8
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(0))? {
            if let Event::Key(key_event) = event::read()? {
                match key_event.code {
                    ...
                    KeyCode::Char('s') => {
                        symmetry_idx = (symmetry_idx + 1) % symmetries.len();
                        let new_config = MandalaConfig {
                            symmetry: symmetries[symmetry_idx],
                            ..config.clone()
                        };
                        viz.update_config(new_config);
                    }
                    KeyCode::Char('+') | KeyCode::Char('=') => {
                        num_layers = (num_layers + 1).min(5);
                        let new_config = MandalaConfig {
                            num_layers,
                            ..config.clone()
                        };
                    ...
                    KeyCode::Char('-') | KeyCode::Char('_') => {
                        num_layers = (num_layers.saturating_sub(1)).max(1);
                        let new_config = MandalaConfig {
                            num_layers,
                            ..config.clone()
                        };
        ...
        // Print status
        print!(
            "\r🕉️  Mandala | Symmetry: {}-fold | Layers: {} | Color: {} | FPS: {:.1} | 'q' quit | 'c' color | 's' symmetry | '+/-' layers",
            symmetries[symmetry_idx],
            num_layers,
