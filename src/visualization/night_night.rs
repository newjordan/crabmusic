...
impl Visualizer for NightNightVisualizer {
    ...
    fn render(&self, grid: &mut GridBuffer) {
        ...
        let pulse =
            (0.5 + 0.5 * (self.time * 0.8).sin()) * (0.4 + 0.6 * (0.4 + self.bass)).min(1.0);
        let color = self
            .color_scheme
            .get_color(pulse)
            .unwrap_or(Color::new(180, 200, 255));
